#math:vec/perf/cross_fvec_base/_print
# 输出测试数据

data modify storage perf:io temp set from storage perf:class math:vec/perf/cross_fvec_base

tellraw @a "--- math:vec/perf/cross_fvec_base perf result ---"
...
# 超过阈值重测
execute if score perf_err int <= perf_check int as @e[tag=perf_v_test,nbt={Tags:["math:vec/perf/cross_fvec_base"]}] run function math:vec/perf/cross_fvec_base/next
execute if score perf_err int <= perf_check int run return 0
tellraw @a "math:vec/perf/cross_fvec_base perf restart"
data modify storage perf:io afk prepend value "math:vec/perf/cross_fvec_base"