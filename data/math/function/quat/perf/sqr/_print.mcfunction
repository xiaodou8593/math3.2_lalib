#math:quat/perf/sqr/_print
# 输出测试数据

data modify storage perf:io temp set from storage perf:class math:quat/perf/sqr

tellraw @a "--- math:quat/perf/sqr perf result ---"
...
# 超过阈值重测
execute if score perf_err int <= perf_check int as @e[tag=perf_v_test,nbt={Tags:["math:quat/perf/sqr"]}] run function math:quat/perf/sqr/next
execute if score perf_err int <= perf_check int run return 0
tellraw @a "math:quat/perf/sqr perf restart"
data modify storage perf:io afk prepend value "math:quat/perf/sqr"