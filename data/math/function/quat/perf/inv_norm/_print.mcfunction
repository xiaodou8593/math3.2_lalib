#math:quat/perf/inv_norm/_print
# 输出测试数据

data modify storage perf:io temp set from storage perf:class math:quat/perf/inv_norm

tellraw @a "--- math:quat/perf/inv_norm perf result ---"
...
# 超过阈值重测
execute if score perf_err int <= perf_check int as @e[tag=perf_v_test,nbt={Tags:["math:quat/perf/inv_norm"]}] run function math:quat/perf/inv_norm/next
execute if score perf_err int <= perf_check int run return 0
tellraw @a "math:quat/perf/inv_norm perf restart"
data modify storage perf:io afk prepend value "math:quat/perf/inv_norm"