#math:quat/perf/inv_norm_compressed/_print
# 输出测试数据

data modify storage perf:io temp set from storage perf:class math:quat/perf/inv_norm_compressed

tellraw @a "--- math:quat/perf/inv_norm_compressed perf result ---"
...
# 超过阈值重测
execute if score perf_err int <= perf_check int as @e[tag=perf_v_test,nbt={Tags:["math:quat/perf/inv_norm_compressed"]}] run function math:quat/perf/inv_norm_compressed/next
execute if score perf_err int <= perf_check int run return 0
tellraw @a "math:quat/perf/inv_norm_compressed perf restart"
data modify storage perf:io afk prepend value "math:quat/perf/inv_norm_compressed"